...
Then update every remaining use of `isComparison` in the same function to `hasOptions`.

**Addendum (found during implementation):** the template's fixed-choice branch was hardcoded to exactly two buttons (`>` and `<`), which happened to work for `comparaison`/`fraction` but would silently break for any future type whose `options` aren't `['>', '<']` (e.g. a `temps` type with textual time options). Generalize it to map over `question.options`, matching the pattern `renderQuestionQcm` already uses:
...
Also in `renderQuestionQcm`, the answer-click handler converts any non-`>`/`<` value with `Number(raw)` — this would corrupt a textual answer like `"14h30"` into `NaN`. Add `const hasOptions = Array.isArray(question.options);` near the top of `renderQuestionQcm` too, and change the handler to:
...
`renderPairsRound` is unchanged — it already works generically off `answer`/`pairKey` values passed in, with no type-name or numeric-conversion check.
